def learn(file_path, file_name, epoch_size, input_size, mid1_size, mid2_size):
    import torch
    ...
    all_labels = (
        df.iloc[0:, input_size : (input_size + 1)].to_numpy().astype(np.float32)
    )
    ...
    class dengue_net(nn.Module):
        def __init__(self):
            ...
            weights = torch.tensor([2.86, 1.54], dtype=torch.float32)
    ...
    def test(model, train_loader):
        ...
        for batch_items, batch_labels in train_loader:
            ...
            _, pred_labels = torch.max(
                outputs, axis=1
            )  # outputsから必要な情報(予測したラベル)のみを取り出す。
    ...
    torch.save(model.state_dict(), file_path.replace(".csv", "_weight.pth"))
    model_scripted = torch.jit.script(model)
